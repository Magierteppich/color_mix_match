...
def KMean_cluster (valid_path, scaled_feature_list): 
    ...
    for cluster in clusters_iter:
        result_dict[f"cluster {cluster}"] = list(cluster_image[cluster][:,0])
...
def show_result_dict_path (path_to_library, height, width):
    ...
    result_dict = KMean_cluster(valid_path, scaled_feature_list)
